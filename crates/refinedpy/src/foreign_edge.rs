...
fn clip_uncarriable_corners(cases: &[ForeignCase]) -> Vec<ForeignCase> {
...
fn interpreter_text_of(interpreter: &Expr, environment: &Environment, kernel: &Arc<RefinedTSKernel>) -> Option<String> {
...
fn const_folded_text_of(expression: &Expr, environment: &Environment, kernel: &Arc<RefinedTSKernel>) -> Option<String> {
...
fn os_system_entry_write_of(statements: &[Stmt], index: usize, infile: &str) -> Option<Expr> {
...
fn os_system_return_read_of(statements: &[Stmt], index: usize, outfile: &str) -> Option<TextRange> {
...
fn literal_open_call_names<'a>(expression: &'a Expr, required_mode: &[&str]) -> Option<&'a str> {
...
fn is_json_load_call(expression: &Expr) -> bool {
    let Expr::Call(call) = expression else {
        return false;
    };
    let Expr::Attribute(attribute) = call.func.as_ref() else {
...
fn redirection_files_of<'a>(remainder: &'a [&'a str]) -> Option<(Option<&'a str>, Option<&'a str>)> {
    match remainder {
        [] => Some((None, None)),
...
fn sole_parse_consumer_from(statements: &[Stmt], result_name: &str, result_read: &ResultRead) -> ParseConsumer {
