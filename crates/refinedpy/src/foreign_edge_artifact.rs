...
/// project-cache path (`cache_artifact_path`'s `.refined/cache/<relpath>
...
/// (`resolve_foreign_producer`'s own `refinedts-check-bin` only exports
/// TypeScript facts) — the cache-path/target-integrity-hash/producer-
/// freshness/auto-export machinery above exists to serve exactly those
/// two premises, neither of which a compiled binary carries. What a
/// compiled binary's fact CAN state is unchanged: the same RULED cases
/// schema (`docs/one-checker/fact-schema.md`'s "The current envelope
/// (ruled)"), read through the SAME field readers this module already
/// applies to a TypeScript artifact (`harness_surface_of`,
/// `function_fact_of` — both already generic over the parsed envelope,
/// with no TypeScript-specific reading inside either), with `language`
/// stating `"cpp"` in place of `"typescript"` and `runtime.band` stating
/// this checker's own compiled-C++ pin.
...
/// Mirrors `FOREIGN_RUNTIME_BAND`'s own role for TypeScript: the SPEC
/// LEVEL the target's checked code runs against, not one compiler binary.
...
pub fn compiled_binary_fact_path(binary_path: &str) -> PathBuf {
...
/// module's doc comment just above `COMPILED_BINARY_ARTIFACT_LANGUAGE`).
...
/// The compiled-binary envelope check — `check_artifact_envelope`'s own
/// twin, admitting `language: "cpp"` in place of `"typescript"` and
