...
use containers::{
    ...
    counter_call, next_call, object_call, set_constructor_call, struct_call_result, vars_call,
};
use conversions::{
...
use numeric::{
    ...
    min_max_over_iterable, min_max_over_star, order_preserving_over_star, reversed_call, round_call, sorted_call,
    sum_call, sum_call_over_star,
};
use stdlib::{copy_call_result, os_call_result, time_call_result};
...
/// The sort word an `open(...)` result carries — a plain noun phrase a
/// fire message can name, the same shape every other `opaque_value` sort
/// in this checker uses. Read by the with-statement's own `__enter__`
/// answer and by `evaluate_attribute_call`'s file-method row, which are
/// the only two places a file object is consumed.
pub const FILE_OBJECT_WORD: &str = "a file object";
...
pub fn builtin_call_result(function: &str, arguments: &[AbstractValue]) -> Option<AbstractValue> {
    match function {
        ...
        "sorted" => sorted_call(arguments).or_else(|| order_preserving_over_star(arguments)),
        "reversed" => reversed_call(arguments).or_else(|| order_preserving_over_star(arguments)),
        ...
        // `open(file, ...)` — library/functions.rst: "Open *file* and
        // return a corresponding file object." The object's own contents
        // are outside this program, so the answer is the honest "the kind
        // of thing is known, its contents are not" shape every other
        // outside-the-program value in this domain takes
        // (`json_loads_value_space`'s own list/dict arms, `re.match`'s
        // match object). Naming the sort is what a `with open(path) as f:`
        // binding needs: the with-statement's own `__enter__` answers the
        // file object itself (functions.rst, "file objects ... are used as
        // context managers"), and `f.read()` then answers the whole-strings
        // ground the file's text is drawn from — A7.seed.boundary's own
        // `file_line_outside`/`file_line_inside` rows, which read `f` as
        // nothing at all before this row existed.
        "open" if !arguments.is_empty() => Some(opaque_value(FILE_OBJECT_WORD)),
        ...
        // `vars(o)` — see `vars_call`'s own doc (library/functions.rst).
        "vars" => vars_call(arguments),
        ...
        // `from collections import Counter` — the same bare-name
        // spelling `quote`/`parse_qs` above take; see `counter_call`'s
        // own doc (library/collections.rst).
        "Counter" => counter_call(arguments),
        _ => None,
    }
}
...
pub fn builtin_call_result_with_kernel(
    ...
    let answer = builtin_call_result_dispatch(function, arguments, kernel);
    // THE BUILTIN-MODELS DISPATCH SEAM (DERIVATION-TRACE.md's own named
    // Rust seam): this dispatcher holds no range of its own — it is
    // called with a name and already-evaluated arguments — so it records
    // onto the span the CALLER opened for the call expression rather than
    // opening one of its own. A modeled row answers; a `None` declines
    // naming the builtin that has no row, which is exactly the construct
    // a reader of the trace needs.
    if crate::trace::is_tracing() {
        match &answer {
            Some(_) => {}
            None => crate::trace::record_decline(
                &format!("no builtin model answers '{function}' for these arguments"),
                None,
                None,
            ),
        }
...
fn builtin_call_result_dispatch(
    function: &str,
    arguments: &[AbstractValue],
    kernel: &Arc<RefinedTSKernel>,
) -> Option<AbstractValue> {
...
pub fn stdlib_call_result(module: &str, function: &str, arguments: &[AbstractValue]) -> Option<AbstractValue> {
    match module {
        ...
        "copy" => copy_call_result(function, arguments),
